...
simple.extravars   <- c("GDP.rate", "pcc.rate")
simple.plotvars    <- c(simple.clustervars, simple.extravars)
simple.f           <- pcc.rate ~ GDP.rate
...
Encouraging.  We don't get any clusters that have all the low-growth or high-growth entries like we did before.  Let's see how the models do.
...
```{r chn.example}
sel   <- simple$table$ISO == "CHN"
clust <- simple$km$cluster[sel]
coef  <- sapply(clust, function(c) {simple$model[[c]]$coefficients["GDP.rate"]})
data.frame(simple$table[sel,c("ISO","year")], cluster=clust, GDP.rate.coef=coef)
```

So, sometime around 2000 China moves from group 4 (roughly speaking:  poor, lightly urbanized countries) to group 1 (roughly speaking:  middle-income, urbanizing countries), and the coefficient for its GDP growth rate becomes a lot smaller.

Note that the coefficient doesn't tell the whole story.  Group 5, comprising the rich countries, has a large coefficient, but its GDP growth rates are lower, so you get slightly lower per-capita cement growth rates in those countries.
